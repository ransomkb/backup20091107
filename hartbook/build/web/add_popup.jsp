...
<%
    try
    {
        ...
        if(request.getParameter("addwordsets") != null)
        {
            String set = request.getParameter("dispvoclist");
            String typ = request.getParameter("type");

            BookMethods.handleSet(set, typ, ss);
	...
	<%= ss.html.mainBodyTopRow() %>
                ...
                <form method='post' name='add' accept-charset="UTF-8">
					<table width='100%'>
						<tr>
							<td>
								<p class='center'><%= ss.exp.getExp9() %></p>
							</td>
							<td class='center'>
							...
							<td class='center'>
								<%= Selections.getSelect3(ss) %>
							</td>
							<td>
								<p class='center'><%= ss.exp.getExp10A() %></p>
							</td>

								<%= Buttons.roundButton(Buttons.getBut10(ss), ss) %>
