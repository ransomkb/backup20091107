...
public class CalculateHistory
{
	...
	public String[] getHistoryList(String userID, SessionState ss)
	{
		...
		clearHistory();
		fetchHistoryList(userID, ss);
		
		for( int i = 2; i < historyList.size(); i++ )
		{
			String s = historyList.get(i).toString();
			String[] histArray = StringHandler.splitSecondString(s);
			
			sumUpHistory(histArray, ss);
		...
		totArray[0] = totList;
		totArray[1] = getTableHTML(userID, ss);
		totArray[2] = getTotalHTML(ss);
		totArray[3] = getHeadHTML(ss);
	...
	private void fetchHistoryList(String userID, SessionState ss)
	{
		try
		{
			...
			else history = clobString(hs, ss);
				//ss.dbgApp("hs clob is "+history); //debug
				
			historyList = StringHandler.splitFirstString(history);
